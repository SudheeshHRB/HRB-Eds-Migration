/**
 * Footer Seals block — S4 Seals
 * TRUSTe seal image + Your Privacy Choices button
 */
export default function decorate(block) {
  ...
  const sealSrc = (data.seal_src
    || '//privacy-policy.truste.com/privacy-seal/seal?rid=d3f53dd3-a8a0-4f4e-84aa-56378ed8565d')
    .replace(/<[^>]+>/g, '').trim();
  const sealHref = (data.seal_href
    || '//privacy.truste.com/privacy-seal/validation?rid=d3f53dd3-a8a0-4f4e-84aa-56378ed8565d')
    .replace(/<[^>]+>/g, '').trim();
  const sealAlt = (data.seal_alt || 'TRUSTe Privacy Certification').replace(/<[^>]+>/g, '').trim();
  const privacyLabel = (data.privacy_label || 'Your Privacy Choices').replace(/<[^>]+>/g, '').trim();
  const privacyHref = (data.privacy_href
